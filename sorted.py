...
setupFiles = ['.java', '.msi', '.iso', '.crdownload']
compressedFiles = ['.zip', '.gz', '.rar', '.tar']
files = ['.apk', '.ppt', '.pptx']
WordsLocation = pathlib.PurePath(currentpath, 'Words')
PdfsLocation = pathlib.PurePath(currentpath, 'Pdfs')
ExcelLocation = pathlib.PurePath(currentpath, 'Excels')
...
for file in filename:
    ...
    if os.path.splitext(file)[1] in excel:
        if(path.exists(ExcelLocation)):
            shutil.move(file, ExcelLocation)
        else:
            os.mkdir(ExcelLocation)
            shutil.move(file, ExcelLocation)
